...
import requests
from .helpers import input_validation, sanitize_input, password_policy, generate_random_filename
	...
	def post(self, request):
		try:
			...
			if (input_validation(username) == False):
				return JsonResponse({"error": "Invalid characters found in username, characters <, >, ', \", & are not allowed"})
	...
	def post(self, request):
		try:
			...
			if (password_policy(password) == False):
				return JsonResponse({'error': 'Password must be at least 8 characters long, with min 1 uppercase, 1 lowercase, 1 number and 1 special character'}, status=400)
...
class GetAllScoresAPICall(APIView):
	...
	def get(self, request):
		try:
			...
			if (r.status_code != 200):
				return JsonResponse({'error': 'Internal Server Error while getting all scores'}, status=500)
